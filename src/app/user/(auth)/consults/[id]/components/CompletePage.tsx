    ...
    <div id="payment-status" className="w-full flex flex-col gap-2 2xl:scale-100 xl:scale-75">
      <div>
        <div id="status-icon" style={{ backgroundColor: STATUS_CONTENT_MAP[status].iconColor }} className="px-4 py-2 flex items-center gap-2 rounded-xl">
          ...
          <h2 id="status-text" className="font-semibold text-white text-lg">{STATUS_CONTENT_MAP[status].text}</h2>
        </div>
      </div>

      {intentId && <div id="details-table" className="border-[1px] rounded-xl px-4 py-2 shadow-sm flex justify-between">
        <h3 className="TableLabel font-semibold opacity-75">Id da transação:</h3>
        <h3 id="intent-id" className="TableContent font-medium">{intentId}</h3>
      </div>}
